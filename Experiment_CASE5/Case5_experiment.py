...
def run_case2_experiment(pre_trained_model_path):
    ...
    csv_file = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE5/Case5_experiment.csv'  # Case 5에 해당하는 데이터 파일
    dataset = pd.read_csv(csv_file)
    
    # 환경 초기화 (33개의 데이터를 사용하는 Case 5 환경 설정)
    ...
    if os.path.exists(pre_trained_model_path):
        ...
        print(f"Loaded trained model from '{pre_trained_model_path}' for Case 5")
    else:
        print(f"No pre-trained model found at '{pre_trained_model_path}', starting from scratch for Case 5")
    ...
    plt.title('Initial Resource Allocation Ratios per Class for Case 2-2')
    plt.xlabel('Class')
...
def main():
    ...
    # Case 2 실험 실행
    print(f"\nStarting experiment for Case 5")
