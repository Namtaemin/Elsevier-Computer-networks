...
def analyze_and_visualize(avg_results_rl, avg_results_dnn, avg_results_pq, priority_ranges):
    ...
    plt.title("Case5: Average Allocated Bandwidth Ratio (RL, DNN, PQ)")
    plt.xlabel("Class")
...
def main():
    ...
    lambdas = [50, 30, 50]
    payload_sizes = [500, 400, 800]
    expected_bandwidths = [0.53, 1.89, 0.81]
    error_margin = 0.05
    priority_ranges = {
        "Class A": (0.9, 1.0),
        "Class B": (0.6, 0.7),
    ...
    # RL Simulation
    env = TSNQoSEnv(traffic_df_copy_rl, n_clusters=num_clusters, initial_data_size=86, initial_bandwidth=140)
    ...
    avg_results_rl = rl_simulation(env, agent, simulation_time, 140)
