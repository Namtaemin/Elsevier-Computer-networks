...
class TSNQoSEnv:
    def __init__(self, dataset, n_clusters=3, initial_data_size=20):
        ...
        # 네트워크 자원 설정
        self.initial_bandwidth = 8000.0  # 제한된 자원 양 설정
        self.max_steps = 1500
    ...
    def get_state(self):
        ...
        state.append(self.initial_bandwidth)  
    ...
    def step(self, action):
        ...
        total_required_resources = sum(class_data['Resource Usage'].sum() for _, class_data in sorted_class_states)
        ...
        for i, (class_index, class_data) in enumerate(sorted_class_states):
            ...
            if i == 0:  # 우선순위가 가장 높은 클래스에 우선적으로 자원 할당
                allocated_resource = min(total_resource_usage, self.usable_resource)
            else:
                satisfaction_rate = max(0.0001, action[i - 1])  # 최소 만족도를 0.01로 설정
        ...
        # 리워드를 계산하는 코드가 calculate_reward 함수로 이동
        done = False
        if self.usable_resource <= 0:
            print("Allocated resources exceeded available resources. Ending episode.")
            done = True
    ...
    def calculate_reward(self, allocated_resources, class_states, action):
        ...
        if self.initial_bandwidth >= total_required_resources:
            for i, (class_name, avg_priority, total_resource_usage) in enumerate(sorted_class_states):
                ...
                if i == 0:
                    if allocated_resource < total_resource_usage:
                        penalty = -0.3 * (total_resource_usage - allocated_resource)  
                        total_reward += penalty
                        print(f"Penalty Applied for {self.class_names[class_name]}: High priority class not fully allocated, Penalty: {penalty}")
                    continue
                ...
                if action_value > prev_action_value and self.usable_resource > 0:
                    ...
                    reward += penalty
                    print(f"Penalty Applied for {self.class_names[class_name]}: Action value lower than previous class, Penalty: {penalty}")
                else:
                    if action_value <= prev_action_value and self.usable_resource > 0:
                ...
                if i < len(sorted_class_states) - 1:
                    ...
                    if action_value < 0.8 and next_action_value > 0.2 and self.usable_resource > 0:
                        penalty = -600 * (next_action_value - 0.2)  # 낮은 우선순위 클래스가 0.2 이상 할당받은 경우 페널티
                        reward += penalty
                        print(f"Penalty Applied for {self.class_names[class_name]}: Lower priority class received more resources than allowed, Penalty: {penalty}")

                total_reward += reward
                total_allocated_resource += allocated_resource
                # print(f"Class {self.class_names[class_name]}: Priority={avg_priority}, Allocated Resource={allocated_resource}, "
                #     f"Total Resource={total_resource_usage}, Step Reward={reward}")
            ...
            for i, (class_name, avg_priority, total_resource_usage) in enumerate(sorted_class_states):
                ...
                if i < len(sorted_class_states) - 1:
                    next_action_value = action[i]
                    if action_value < 0.8 and next_action_value > 0.2 and self.usable_resource > 0:
                        penalty = -400 * (next_action_value - 0.2)  # 낮은 우선순위 클래스가 0.2 이상 할당받은 경우 페널티
                        reward += penalty
                        print(f"Penalty Applied for {self.class_names[class_name]}: Lower priority class received more resources than allowed, Penalty: {penalty}")
    ...
    def manage_data_flow(self):
        ...
        self.usable_resource = self.initial_bandwidth - all_data['Resource Usage'].sum()
