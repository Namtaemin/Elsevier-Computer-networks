...
def run_case1_experiment(pre_trained_model_path):
    ...
    csv_file = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE1/Case1_experiment.csv'  # Case 1에 해당하는 데이터 파일
    dataset = pd.read_csv(csv_file)
    
    # 환경 초기화 (33개의 데이터를 사용하는 Case 1 환경 설정)
    env = TSNQoSEnv(dataset, n_clusters=3, initial_data_size=33, initial_bandwidth=132)  # 초기 데이터 수를 33으로 설정
    ...
    plt.title('Initial Resource Allocation Ratios per Class for Case 1 (Sorted by Priority)')
    plt.xlabel('Class')
...
def main():
    ...
    # Case 2 실험 실행
    print(f"\nStarting experiment for Case 1")
    class_resource_allocation, total_reward = run_case1_experiment(pre_trained_model_path)
