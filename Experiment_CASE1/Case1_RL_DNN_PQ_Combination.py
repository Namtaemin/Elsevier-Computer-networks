...
def analyze_and_visualize(avg_results_rl, avg_results_dnn, avg_results_pq, priority_ranges):
    ...
    # Move Class C results to Class B for RL
    if "Class B" in avg_results_rl["Class"].values and avg_results_rl.loc[avg_results_rl["Class"] == "Class B", "Resource Usage"].values[0] == 0:
        # Copy Class C data to Class B
        avg_results_rl.loc[avg_results_rl["Class"] == "Class B", ["Allocated Bandwidth", "Resource Usage", "Allocation Percentage"]] = \
            avg_results_rl.loc[avg_results_rl["Class"] == "Class C", ["Allocated Bandwidth", "Resource Usage", "Allocation Percentage"]].values[0]
        # Remove Class C
        avg_results_rl = avg_results_rl[avg_results_rl["Class"] != "Class C"]

    # Sort results by priority order
    priority_order = ["Class A", "Class B", "Class C"]  # Fix priority order explicitly

    # Sort each result set by the fixed priority order
    avg_results_rl = avg_results_rl.set_index("Class").reindex(priority_order).reset_index()
    ...
    plt.title("Case1: Average Allocated Bandwidth Ratio (RL, DNN, PQ)")
    plt.xlabel("Class")
...
def main():
    ...
    pre_trained_model_path = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE3/trained_policy_model_Case3.pth'
