...
from EA_6_Class_env import TSNQoSEnv
import os
import Config
...
def analyze_and_visualize(avg_results_rl, avg_results_dnn, avg_results_pq, priority_ranges):
    ...
    manual_order = ["Class A", "Class B", "Class C", "Class D", "Class E", "Class F"]
    ...
    plt.title("Case2: Average Allocated Bandwidth Ratio (RL, DNN, PQ)")
    plt.xlabel("Class")
...
def main():
    """
    CASE2 RL, DNN, PQ 통합 시뮬레이션 및 분석
    """
    # CASE2 설정
    simulation_time = 100
    classes = ["Class A", "Class B", "Class C", "Class D", "Class E", "Class F"]
    lambdas = [40, 70, 25, 30, 15, 10]
    payload_sizes = [500, 400, 300, 200, 600, 700]
    expected_bandwidths = [0.53, 1.89, 0.43, 1.12, 0.94, 2.05]
    error_margin = 0.05
    priority_ranges = {
        "Class A": (0.9, 1.0),
        "Class B": (0.7, 0.8),
    ...
    is_zero_bandwidth = [False, False, False, False, False, False]  # Class F의 대역폭은 항상 0
    batch_bandwidth = 149.765  # 총 대역폭
    num_clusters = 6
    ...
    env = TSNQoSEnv(traffic_df_copy_rl, n_clusters=num_clusters, initial_data_size=190, initial_bandwidth=batch_bandwidth)
    agent = Agent(15, action_size=Config.ACTION_SIZE, batch_size=Config.BATCH_SIZE)
    rl_model_path = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE2/trained_policy_model_Case2.pth'
    if os.path.exists(rl_model_path):
        agent.agent_control.policy_nn.load_state_dict(torch.load(rl_model_path))
    else:
        print(f"No pre-trained model found at '{rl_model_path}'")
        return
    avg_results_rl = rl_simulation(env, agent, simulation_time, batch_bandwidth)

    # DNN Simulation
    traffic_df_copy_dnn = traffic_df.copy()
    dnn_model_path = '/home/scpa/Policy_DDS_TSN/sin/DNN_regression/Experiment_CASE2/DNN_Class63.h5'
    dnn_model = load_model(dnn_model_path)
    avg_results_dnn = dnn_simulation(dnn_model, traffic_df_copy_dnn, batch_bandwidth, simulation_time)
