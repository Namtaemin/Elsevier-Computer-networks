...
def run_case3_experiment(pre_trained_model_path):
    # 33개의 데이터를 포함한 CSV 파일 로드
    csv_file = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE3/Case3_experiment.csv'  # Case 2에 해당하는 데이터 파일
    dataset = pd.read_csv(csv_file)
    
    # 환경 초기화 (33개의 데이터를 사용하는 Case 3 환경 설정)
    ...
    plt.title('Initial Resource Allocation Ratios per Class for Case 1-3')
    plt.xlabel('Class')
...
def main():
    ...
    # Case 2 실험 실행
    print(f"\nStarting experiment for Case 3")
    class_resource_allocation, total_reward = run_case3_experiment(pre_trained_model_path)
