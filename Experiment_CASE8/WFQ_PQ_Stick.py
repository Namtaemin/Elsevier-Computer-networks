...
def run_case1_experiment(pre_trained_model_path):
    csv_file = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE2/Case2_experiment.csv'
    dataset = pd.read_csv(csv_file)
    
    env = TSNQoSEnv(dataset, n_clusters=6, initial_data_size=100, initial_bandwidth=70)
    ...
    if os.path.exists(pre_trained_model_path):
        ...
        print(f"Loaded trained model from '{pre_trained_model_path}' for Case 1-3")
    else:
        print(f"No pre-trained model found at '{pre_trained_model_path}', starting from scratch for Case 1-2")
        return None
...
def wfq_algorithm(data, original_data):
    ...
    allocation_ratios = {}
    allocated_resources = {f'class_{i}': 0 for i in range(6)}
    ...
    for _, row in sorted_data.iterrows():
        ...
        weight = row['Priority'] / total_priority if total_priority > 0 else 1 / 6
...
def main():
    ...
    rule_based_csv = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE2/Case2_experiment.csv'
    
    num_experiments = 20
    all_data = {
        'RL': {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': [], 'class_4': [], 'class_5': []},
        'WFQ': {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': [], 'class_4': [], 'class_5': []},
        'PQ': {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': [], 'class_4': [], 'class_5': []}
    }
