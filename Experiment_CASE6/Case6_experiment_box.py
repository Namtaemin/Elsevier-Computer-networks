...
def run_case1_experiment(pre_trained_model_path):
    ...
    env = TSNQoSEnv(dataset, n_clusters=3, initial_data_size=54, initial_bandwidth=272)
    ...
    if os.path.exists(pre_trained_model_path):
        ...
        print(f"Loaded trained model from '{pre_trained_model_path}' for Case 6")
    else:
        print(f"No pre-trained model found at '{pre_trained_model_path}', starting from scratch for Case 6")
...
def main():
    pre_trained_model_path = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE6/trained_policy_model_Case1.pth'
    
    num_experiments = 5
    all_runs_ratios = {'class_0': [], 'class_1': [], 'class_2': []}  # 클래스별 할당 비율 저장
    all_class_priorities = {'class_0': [], 'class_1': [], 'class_2': []}  # 클래스별 우선순위 저장
    ...
    bars = plt.bar(class_names, avg_ratios.values(), color='orange', yerr=yerr, capsize=5, alpha=0.6)
    ...
    plt.title(f'Case2-3 Resource Allocation Ratios with Min/Max/Median')
    plt.xlabel('Class')
