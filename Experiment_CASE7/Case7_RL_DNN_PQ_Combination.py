...
def analyze_and_visualize(avg_results_rl, avg_results_dnn, avg_results_pq, priority_ranges):
    ...
    plt.title("Case7: Average Allocated Bandwidth Ratio (RL, DNN, PQ)")
    plt.xlabel("Class")
...
def main():
    ...
    expected_bandwidths = [0.53, 1.89, 0.83, 0.21]
    error_margin = 0.05
    priority_ranges = {
        "Class A": (0.9, 1.0),
        "Class B": (0.6, 0.8),
    ...
    env = TSNQoSEnv(traffic_df_copy_rl, n_clusters=num_clusters, initial_data_size=150, initial_bandwidth=90.07)
    ...
    if os.path.exists(rl_model_path):
        ...
        return
    avg_results_rl = rl_simulation(env, agent, simulation_time, 90.07)
