...
def run_case4_experiment(pre_trained_model_path):
    # 33개의 데이터를 포함한 CSV 파일 로드
    csv_file = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE4/Case4_experiment.csv'  # Case 2에 해당하는 데이터 파일
    dataset = pd.read_csv(csv_file)
    
    # 환경 초기화 (33개의 데이터를 사용하는 Case 4 환경 설정)
    env = TSNQoSEnv(dataset, n_clusters=4, initial_data_size=65, initial_bandwidth=150)
    ...
    if os.path.exists(pre_trained_model_path):
        ...
        print(f"Loaded trained model from '{pre_trained_model_path}' for Case 4")
    else:
        print(f"No pre-trained model found at '{pre_trained_model_path}', starting from scratch for Case 4")
    ...
    plt.bar(class_names, sorted_allocation_ratios, color='orange')
    plt.title('Initial Resource Allocation Ratios per Class for Case 2-1')
    plt.xlabel('Class')
...
def main():
    # 모델 파일 경로 (Case 4 전용)
    pre_trained_model_path = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE4/trained_policy_model_Case4.pth'
    
    # Case 4 실험 실행
    print(f"\nStarting experiment for Case 4")
    class_resource_allocation, total_reward = run_case4_experiment(pre_trained_model_path)
