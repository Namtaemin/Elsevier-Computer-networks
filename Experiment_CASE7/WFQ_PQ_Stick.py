...
def run_case1_experiment(pre_trained_model_path):
    csv_file = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE4/Case4_experiment.csv'
    dataset = pd.read_csv(csv_file)
    
    env = TSNQoSEnv(dataset, n_clusters=4, initial_data_size=65, initial_bandwidth=140)
    ...
    if os.path.exists(pre_trained_model_path):
        ...
        print(f"Loaded trained model from '{pre_trained_model_path}' for Case 2-1")
    else:
        print(f"No pre-trained model found at '{pre_trained_model_path}', starting from scratch for Case 2-1")
        return None
...
def wfq_algorithm(data, original_data):
    ...
    allocation_ratios = {}
    allocated_resources = {f'class_{i}': 0 for i in range(4)}
    ...
    for _, row in sorted_data.iterrows():
        ...
        weight = row['Priority'] / total_priority if total_priority > 0 else 1 / 4
...
def main():
    pre_trained_model_path = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE4/trained_policy_model_Case3.pth'
    rule_based_csv = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE4/Case4_experiment.csv'
    
    num_experiments = 20
    all_data = {
        'RL': {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': []},
        'WFQ': {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': []},
        'PQ': {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': []}
    }
