...
from EA_5_Class_env import TSNQoSEnv  # 환경 파일이 TSNQoSEnv.py에 있다고 가정
...
def run_case1_experiment(pre_trained_model_path):
    ...
    env = TSNQoSEnv(dataset, n_clusters=5, initial_data_size=172, initial_bandwidth=650)  # 초기 데이터 수를 33으로 설정
...
def main():
    ...
    num_experiments = 30
    all_runs_ratios = {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': [], 'class_4': []}  # 클래스별 할당 비율 저장
    all_class_priorities = {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': [], 'class_4': []}  # 클래스별 우선순위 저장
    ...
    bars = plt.bar(class_names, avg_ratios.values(), color='blue', yerr=yerr, capsize=5, alpha=0.6)
    ...
    plt.title(f'Case3 Resource Allocation Ratios with Min/Max/Median')
    plt.xlabel('Class')
