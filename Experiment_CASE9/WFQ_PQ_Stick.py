...
from EA_5_Class_env import TSNQoSEnv  # 환경 파일이 5개의 클래스로 설정되어 있다고 가정
...
def run_case1_experiment(pre_trained_model_path):
    csv_file = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE7/Case7_experiment.csv'
    dataset = pd.read_csv(csv_file)
    
    env = TSNQoSEnv(dataset, n_clusters=5, initial_data_size=172, initial_bandwidth=650)
    ...
    if os.path.exists(pre_trained_model_path):
        ...
        print(f"Loaded trained model from '{pre_trained_model_path}' for Case 7")
    else:
        print(f"No pre-trained model found at '{pre_trained_model_path}', starting from scratch for Case 7")
        return None
...
def wfq_algorithm(data, original_data):
    ...
    allocation_ratios = {}
    allocated_resources = {f'class_{i}': 0 for i in range(5)}
    ...
    for _, row in sorted_data.iterrows():
        ...
        weight = row['Priority'] / total_priority if total_priority > 0 else 1 / 5
...
def main():
    ...
    rule_based_csv = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE7/Case7_experiment.csv'
    
    num_experiments = 20
    all_data = {
        'RL': {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': [], 'class_4': []},
        'WFQ': {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': [], 'class_4': []},
        'PQ': {'class_0': [], 'class_1': [], 'class_2': [], 'class_3': [], 'class_4': []}
    }
