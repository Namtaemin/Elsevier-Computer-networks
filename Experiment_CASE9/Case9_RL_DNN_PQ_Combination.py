...
from EA_5_Class_env import TSNQoSEnv
import os
import Config
...
def analyze_and_visualize(avg_results_rl, avg_results_dnn, avg_results_pq, priority_ranges):
    ...
    manual_order = ["Class A", "Class B", "Class C", "Class D", "Class E"]
    ...
    plt.title("Case9: Average Allocated Bandwidth Ratio (RL, DNN, PQ)")
    plt.xlabel("Class")
...
def main():
    ...
    lambdas = [40, 40, 20, 15, 10]
    payload_sizes = [500, 400, 300, 200, 600]
    expected_bandwidths = [0.53, 1.89, 0.83, 0.12, 0.05]
    error_margin = 0.05
    priority_ranges = {
        "Class A": (0.9, 1.0),
        "Class B": (0.7, 0.8),
    ...
    is_zero_bandwidth = [False, False, False, False, False]  # Class F의 대역폭은 항상 0
    batch_bandwidth = 104.35 # 총 대역폭
    num_clusters = 5
    ...
    env = TSNQoSEnv(traffic_df_copy_rl, n_clusters=num_clusters, initial_data_size=125, initial_bandwidth=batch_bandwidth)
    agent = Agent(15, action_size=Config.ACTION_SIZE, batch_size=Config.BATCH_SIZE)
    rl_model_path = '/home/scpa/Policy_DDS_TSN/sin/PPO-RL-base/Experiment_CASE9/trained_policy_model_Case7.pth'
