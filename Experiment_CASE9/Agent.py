...
class Agent:
    ...
    def get_action_quantization(self, state):
        actions, actions_logprob, mean, std = self.agent_control.get_action_quantization(state)
        return actions.cpu().detach().numpy(), actions_logprob, mean, std
...
class AgentControl:
    ...
    def get_action(self, state):
        actions, actions_logprob, _, mean, std = self.policy_nn(state.clone().detach().to(self.device))
